...
SUITES="tests/test-webhook-provisioning.php tests/test-refund.php tests/test-email-branding.php tests/test-log-redaction.php"
...
exec docker run --rm -v "/$(pwd)://app" -w //app php:8.2-cli \
    sh -c 'FAILED=0; for s in tests/test-webhook-provisioning.php tests/test-refund.php tests/test-email-branding.php tests/test-log-redaction.php; do php "$s" || FAILED=1; done; exit $FAILED'

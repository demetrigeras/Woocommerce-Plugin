...
SUITES="tests/test-webhook-provisioning.php tests/test-refund.php"
...
exec docker run --rm -v "/$(pwd)://app" -w //app php:8.2-cli \
    sh -c 'FAILED=0; for s in tests/test-webhook-provisioning.php tests/test-refund.php; do php "$s" || FAILED=1; done; exit $FAILED'
